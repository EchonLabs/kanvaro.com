...
A **scheduled cron job** that runs every minute to automatically stop timers that exceed their `maxSessionHours` limit.
...
5. **New: `/vercel.json`**
   ...
   - Sets timer-cleanup to run every minute
...
1. **Every minute**, the cron job runs automatically
...
- User starts timer at 8:00 AM (maxSessionHours = 4)
- User closes laptop at 10:00 AM
- Cron runs within 1 minute after 4-hour limit
...
| User offline, timer hits limit | ❌ Keeps running | ✅ Stops within 1 min (cron) |
...
5. ✅ **Low impact** - Runs every minute, minimal database load
