...
function sanitizePastedHtml(html: string): string {
  ...
  for (const el of elements) {
    ...
    // BUT: preserve formatting attributes on SPAN (style) and FONT (size, face, color) for proper formatting preservation
    for (const attr of Array.from(el.attributes)) {
      const name = attr.name.toLowerCase();
      // Safe attributes that should always be kept
      ...
      // NEW: Allow formatting attributes on SPAN for inline styling (colors, font-size, font-family, etc.)
      if (name === "style" && tag === "SPAN") continue;
      // NEW: Allow formatting attributes on FONT for legacy execCommand output
      if (
        (name === "size" || name === "face" || name === "color") &&
        tag === "FONT"
      )
        continue;
      // All other attributes get stripped
    ...
    // CHANGED: Preserve SPAN and FONT tags instead of unwrapping them.
    // These tags are used by execCommand for formatting and contain important style information.
    // We keep them so that font-size, font-family, and text colors are preserved.
    if (tag === "SPAN" && !el.getAttribute("style")) {
      // Only unwrap SPAN if it has no style attribute (truly empty formatting wrapper)
      const parent = el.parentNode;
      if (!parent) continue;
      while (el.firstChild) parent.insertBefore(el.firstChild, el);
      parent.removeChild(el);
    ...
    if (
      ...
      // Only unwrap FONT if it has no formatting attributes
      const parent = el.parentNode;
      if (!parent) continue;
      while (el.firstChild) parent.insertBefore(el.firstChild, el);
      parent.removeChild(el);
    ...
    // If SPAN has style or FONT has size/face/color, keep the tag (don't unwrap)
  ...
  container.querySelectorAll("p,h1,h2,h3,h4,h5,h6,li").forEach((block) => {
    ...
    const brOnly =
      block.querySelectorAll("*").length === 1 && block.querySelector("br");
    if (
      !hasImage &&
      (!text || brOnly) &&
      block.querySelectorAll("br").length > 0
    ) {
      block.remove();
    } else if (
      !hasImage &&
...
async function uploadImageToAttachmentsApi(
  file: Blob,
  filename: string,
...
async function replaceDataUrlImagesWithUploads(html: string): Promise<string> {
  ...
  for (const img of images) {
    ...
    try {
      ...
      const uploaded = await uploadImageToAttachmentsApi(
        blob,
        `pasted-image.${ext}`,
      );
      if (uploaded?.url) {
        img.setAttribute("src", uploaded.url);
        if (uploaded.name && !img.getAttribute("alt"))
          img.setAttribute("alt", uploaded.name);
      } else {
...
export function RichTextEditor({
    ...
    async (e: React.ClipboardEvent) => {
      ...
      if (imageItem) {
        ...
        if (html && html.trim()) {
          ...
          setIsUploadingImage(true);
          try {
            const htmlWithUploadedImages =
              await replaceDataUrlImagesWithUploads(html);
  ...
  const [selectedFontFamily, setSelectedFontFamily] =
    useState<string>("Arial, sans-serif"); // Default to Arial

  const increaseFontSize = useCallback(() => {
    if (disabled) return;

    const currentIndex = fontSizes.findIndex(
      (size) => size.value === selectedFontSize,
    );
  ...
  const setImageAlignment = useCallback(
    (img: HTMLImageElement, alignment: "left" | "center" | "right") => {
  ...
  return (
    <div
      className={cn("border rounded-md overflow-visible relative", className)}
    >
      {/* Toolbar */}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="z-[10050]">
            <DropdownMenuItem onClick={() => insertUnorderedList("disc")}>
              <span
                className="w-4 h-4 rounded-full bg-current mr-2"
            ...
            <DropdownMenuItem onClick={() => insertUnorderedList("circle")}>
              <span
                className="w-4 h-4 rounded-full border border-current mr-2"
            ...
            <DropdownMenuItem onClick={() => insertUnorderedList("square")}>
              <span
                className="w-4 h-4 bg-current mr-2"
                style={{ listStyleType: "square", width: "6px", height: "6px" }}
              ></span>
              Square
            </DropdownMenuItem>
          </DropdownMenuContent>
      ...
      {showCharCount && maxLength && (
        <div className="px-3 py-2 border-t bg-muted/30 text-xs text-muted-foreground flex justify-between items-center">
          ...
          <span
            className={cn(
              "text-xs font-medium",
              getCharCount(value) > maxLength * 0.9
                ? "text-destructive"
                : getCharCount(value) > maxLength * 0.8
                  ? "text-yellow-600"
