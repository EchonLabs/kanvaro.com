  ...
  try {
    ...
    for (let taskIdx = 0; taskIdx < tasksToCreate.length; taskIdx++) {
      ...
      if (!counter) {
        try {
          ...
          if (!counter) {
            throw new Error('Counter creation returned null')
          }
        ...
        try {
          counter.seq += 1
          await counter.save()
          console.log(`[bulk-create] Counter incremented to seq=${counter.seq}`)
        } catch (savErr) {
          console.error(`[bulk-create] Failed to save counter:`, savErr)
          throw savErr
        }
      }
    ...
    for (const projectId of projectIdsArray) {
      try {
        ...
        console.log(`[bulk-create] ✓ Cache invalidated for project ${projectId}`)
      } catch (cacheErr) {
        console.error(`[bulk-create] ⚠ Cache invalidation failed for project ${projectId}:`, cacheErr)
    ...
    for (const orgId of Array.from(affectedOrganizations)) {
      try {
        ...
        console.log(`[bulk-create] ✓ Cache invalidated for org ${orgId}`)
      } catch (cacheErr) {
        console.error(`[bulk-create] ⚠ Cache invalidation failed for org ${orgId}:`, cacheErr)
        // Don't throw - cache failure shouldn't block response
      }
    }
