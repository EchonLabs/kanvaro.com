...
import { Download, FileText, Loader2, Plus, Search, ShieldCheck, Trash2, UploadCloud, Eye } from 'lucide-react'
...
export default function DocumentTemplatesPage() {
  ...
  const [previewDoc, setPreviewDoc] = useState<DocumentTemplateItem | null>(null)
  ...
  const handlePreview = async (doc: DocumentTemplateItem) => {
    setPreviewDoc(doc)
    setPreviewOpen(true)
  ...
  const renderPreviewContent = () => {
    ...
    const { mimeType, fileUrl, name } = previewDoc
    const isDocx =
      mimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ||
      previewDoc.fileName.toLowerCase().endsWith('.docx')
    ...
    if (isDocx && previewHtml) {
      return (
        <iframe
          srcDoc={`<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{font-family:system-ui,sans-serif;line-height:1.6;color:#222;max-width:850px;margin:0 auto;padding:32px}img{max-width:100%;height:auto}table{border-collapse:collapse;max-width:100%}td,th{border:1px solid #ccc;padding:6px 10px}</style></head><body>${previewHtml}</body></html>`}
          sandbox=""
    ...
    // Fallback for other file types (Word, Excel, plain text, etc.)
    return (
      <div className="flex h-full flex-col items-center justify-center text-center">
    ...
    <MainLayout>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex h-8 items-center gap-1 rounded-[10px] bg-[var(--apple-card-gradient)] px-3 text-[12px] font-medium text-white shadow-sm transition-opacity hover:opacity-90"
                        >
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl">
          ...
          <div className="h-[70vh] min-h-[400px] w-full overflow-hidden rounded-md bg-muted/20">
