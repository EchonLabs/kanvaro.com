...
export async function processTimerCleanup() {
  await connectDB()
  ...
  if (!activeTimers || activeTimers.length === 0) {
    return {
      ...
      summary: { totalChecked: 0, stopped: 0, skipped: 0, errors: 0 },
      results: []
    }
  }
  ...
  for (const timer of activeTimers) {
    try {
      ...
      if (!exceeded && settings.maxDailyHours) {
        const timerUserId = getIdString(timer.user)
        if (timerUserId) {
          const dailyHoursLogged = await getDailyHoursLogged(timerUserId, organizationId)
          const currentSessionHours = currentDuration / MINUTES_PER_HOUR
          if (dailyHoursLogged + currentSessionHours >= settings.maxDailyHours) {
            exceeded = true
          }
