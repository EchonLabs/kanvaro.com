/**
 * Assembling a real `CompletionContext` from the database (RUN-19..22, §17.6).
 *
 * Originally lived inline in `complete/route.ts`, per that route's own
 * docblock note that the assembly was route-local "if a second caller ever
 * needs it, promote it then." Task 12 (E49 backfill) is that second caller —
 * a backfill is functionally "run the completion saga against a `Missed`
 * standup instead of an `In_Progress` one," so it needs the exact same
 * `CompletionContext` shape, built from the same loader queries. Promoted
 * here so both `POST /api/standups/:id/complete` and
 * `POST /api/standups/:id/backfill` build it identically rather than
 * maintaining two copies that could drift.
 *
 * Two things this assembly must get right that are easy to get quietly wrong
 * (carried over from the original docblock):
 *
 ...
 * from `freeze-allocations` instead of resuming. So this reads
 * `standup.completionState?.runId` back off the document first, and only
 * mints a fresh id when there is none.
 *
 ...
 */
export async function assembleCompletionContext(input: {
  standupId: string
  standup: any
  projectId: string
  ...
  // string id the rest of this module (and `BuildSummaryInput`'s own Mixed
