...
<body>
  ...
  generateAIProjectTrackingEmail(
    projectName: string,
    ...
    const safeFrom = new Date(dateFrom).toLocaleDateString()
    const safeTo = new Date(dateTo).toLocaleDateString()
    const safeProject = escapeHtml(projectName)
    const logoUrl = resolveEmailLogoUrl(COMPANY_LOGO_URL)

    const reportHtml = reportMarkdown
      .replace(/^## (.+)$/gm, '<h2 style="color:#1e293b;font-size:17px;font-weight:700;margin:24px 0 8px;border-bottom:1px solid #e2e8f0;padding-bottom:6px;">$1</h2>')
      .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
      .replace(/^- (.+)$/gm, '<li style="margin:4px 0;color:#374151;">$1</li>')
      .replace(/(<li[^>]*>.*<\/li>\n?)+/g, (match) => `<ul style="margin:8px 0 16px 20px;padding:0;">${match}</ul>`)
      .replace(/\n\n/g, '</p><p style="color:#374151;margin:8px 0;">')
      .replace(/^(?!<[hul])(.+)$/gm, '<p style="color:#374151;margin:8px 0;">$1</p>')
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>AI Project Tracking Report — ${safeProject}</title>
</head>
<body style="margin:0;padding:24px 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f3f4f6;color:#111827;">
  <div style="max-width:640px;margin:0 auto;background:#fff;border-radius:16px;padding:40px 36px;box-shadow:0 18px 45px rgba(15,23,42,0.15);">
    <div style="text-align:center;margin-bottom:32px;">
      <img src="${logoUrl}" alt="Kanvaro" width="140" style="display:block;margin:0 auto 20px;" />
      <div style="display:inline-block;background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%);color:#fff;padding:10px 20px;border-radius:20px;font-size:13px;font-weight:600;letter-spacing:0.5px;">AI Project Tracking Report</div>
      <h1 style="color:#111827;font-size:26px;font-weight:700;margin:16px 0 6px;">${safeProject}</h1>
      <p style="color:#6b7280;font-size:14px;margin:0;">${safeFrom} — ${safeTo} &bull; ${standupCount} standups analyzed</p>
    </div>
    <div style="font-size:15px;line-height:1.7;">${reportHtml}</div>
    <div style="margin-top:36px;padding-top:24px;border-top:1px solid #e5e7eb;text-align:center;color:#9ca3af;font-size:13px;">
      <p style="margin:0;">Generated by Kanvaro AI &bull; ${organizationName}</p>
    </div>
  </div>
</body>
</html>`
  ...
  generateAIPerformanceReportEmail(
    memberName: string,
    projectName: string,
    dateFrom: Date | string,
    dateTo: Date | string,
    ...
    const reportHtml = reportMarkdown
      .replace(/^## (.+)$/gm, '<h2 style="color:#1e293b;font-size:18px;margin:24px 0 8px;border-bottom:1px solid #e2e8f0;padding-bottom:6px;">$1</h2>')
      .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
      .replace(/^- (.+)$/gm, '<li style="margin:4px 0;color:#374151;">$1</li>')
      .replace(/(<li[^>]*>.*<\/li>\n?)+/g, (match) => `<ul style="margin:8px 0 16px 20px;padding:0;">${match}</ul>`)
      .replace(/\n\n/g, '</p><p style="color:#374151;margin:8px 0;">')
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Performance Report — ${safeProject}</title>
</head>
<body style="margin:0;padding:24px 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f3f4f6;color:#111827;">
  <div style="max-width:640px;margin:0 auto;background:#fff;border-radius:16px;padding:40px 36px;box-shadow:0 18px 45px rgba(15,23,42,0.15);">
    <div style="text-align:center;margin-bottom:32px;">
      <img src="${logoUrl}" alt="Kanvaro" width="140" style="display:block;margin:0 auto 20px;" />
      <div style="display:inline-block;background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%);color:#fff;padding:10px 20px;border-radius:20px;font-size:13px;font-weight:600;letter-spacing:0.5px;">AI-Generated Performance Report</div>
      <h1 style="color:#111827;font-size:26px;font-weight:700;margin:16px 0 6px;">${safeProject}</h1>
      <p style="color:#6b7280;font-size:14px;margin:0;">${safeFrom} — ${safeTo} &bull; ${standupCount} standups analyzed</p>
    </div>

    <div style="background:#f8fafc;border-left:4px solid #6366f1;border-radius:6px;padding:16px 20px;margin-bottom:28px;">
      <p style="margin:0;font-size:15px;color:#374151;">Hi <strong>${safeName}</strong>, here is your AI-generated performance report for the <strong>${safeProject}</strong> project based on the last ${standupCount} standup sessions.</p>
    </div>
    ...
    <div style="margin-top:36px;padding-top:24px;border-top:1px solid #e5e7eb;text-align:center;color:#9ca3af;font-size:13px;">
      <p style="margin:0 0 8px;">This report was generated by Kanvaro AI and is based on standup activity data.</p>
      <p style="margin:0;">If you have questions, reach out to your project manager.</p>
    </div>
  </div>
</body>
</html>`
